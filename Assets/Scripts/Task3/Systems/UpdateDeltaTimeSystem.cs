...
namespace Task3.Systems
{
    /*Used for testing as a device to query entities whether they should update current frame
     I didn't include this in solution as it had no effect and instead produced
     a substantial overhead on its own, with data oriented ecs it is often more efficient to perform
     even not needed calculations en masse than to block the command buffer with structural changes.
     I left it as a proof for eventual testing by the person who will evaluate my submission
     Entities movement code didn't require any additional optimizations, with 100 000 units on my machine
     the ContinuousTranslationSystem takes on average 0.02 ms.*/
